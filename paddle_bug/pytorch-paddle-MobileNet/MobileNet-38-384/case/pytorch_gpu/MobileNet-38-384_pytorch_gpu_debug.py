...
output_dir = "./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/"
...
class Model_Vusb5Wp1jkUTCwGcnJU8w7MOaJIpP9tZ(nn.Module):
    ...
    def forward(self, input):
        ...
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/conv5_output.npz",conv5_output.cpu().detach().numpy())
        relu4_output = self.relu4_mutated(conv5_output)
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/relu4_output.npz",relu4_output.cpu().detach().numpy())
        ...
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/conv19_output.npz",conv19_output.cpu().detach().numpy())
        relu18_output = self.relu18_mutated(conv19_output)
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/relu18_output.npz",relu18_output.cpu().detach().numpy())
        conv20_output = self.conv20_mutated(relu18_output)
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/conv20_output.npz",conv20_output.cpu().detach().numpy())
        tail_flatten_output = self.tail_flatten(conv20_output)
        print("tail_flatten_output: ", tail_flatten_output)
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/tail_flatten_output.npz",tail_flatten_output.cpu().detach().numpy())
        tail_fc_output = self.tail_fc(tail_flatten_output)
        # print(tail_fc_output.shape)
        np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/tail_fc_output.npz",tail_fc_output.cpu().detach().numpy())
...
# print(output)
np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/layer_outputs/output.npz",output)
np.savez("./pytorch-paddle-MobileNet/MobileNet-38-384/case/pytorch_gpu/grad-new.npz", **gradients)