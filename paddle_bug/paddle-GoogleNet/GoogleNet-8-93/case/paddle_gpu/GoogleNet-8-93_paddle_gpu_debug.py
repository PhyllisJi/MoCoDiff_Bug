...
output_dir = "./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/layer_outputs/"
os.makedirs(output_dir, exist_ok=True)
class Model_1716343434(nn.Layer):
    def __init__(self):
        super(Model_1716343434, self).__init__()
    ...
    def forward(self, input):
        ...
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/layer_outputs/conv3_output.npz",conv3_output.cpu().detach().numpy())
        relu3_output = self.relu3_mutated(conv3_output)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/layer_outputs/relu3_output.npz",relu3_output.cpu().detach().numpy())
        inception1_input = self.pool2(relu3_output)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/layer_outputs/inception1_input.npz",inception1_input.cpu().detach().numpy())
        tail_flatten_output = self.tail_flatten(inception1_input)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/layer_outputs/tail_flatten_output.npz",tail_flatten_output.cpu().detach().numpy())
...
inp = np.load("./paddle-GoogleNet/GoogleNet-8-93/case/input.npz")
gradients, loss, output = train(inp['inp'], inp['label'])
print(output)
np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_gpu/grad-new.npz", **gradients)