...
output_dir = "./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/layer_outputs/"
...
class Model_1716690305(nn.Layer):
    ...
    def forward(self, input):
        ...
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/layer_outputs/conv3_output.npz",conv3_output.cpu().detach().numpy())
        relu3_output = self.relu3_mutated(conv3_output)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/layer_outputs/relu3_output.npz",relu3_output.cpu().detach().numpy())
        inception1_input = self.pool2(relu3_output)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/layer_outputs/inception1_input.npz",inception1_input.cpu().detach().numpy())
        tail_flatten_output = self.tail_flatten(inception1_input)
        np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/layer_outputs/tail_flatten_output.npz",tail_flatten_output.cpu().detach().numpy())
...
print(output)
np.savez("./paddle-GoogleNet/GoogleNet-8-93/case/paddle_cpu/grad-new.npz", **gradients)