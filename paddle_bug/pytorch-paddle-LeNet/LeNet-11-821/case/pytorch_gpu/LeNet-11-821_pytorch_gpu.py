...
class Model_WsMsDPQSJciomCH995XFEZngH7RTSMqu(nn.Module):
    def __init__(self):
        super(Model_WsMsDPQSJciomCH995XFEZngH7RTSMqu, self).__init__()
        ...
        self.conv2_mutated = torch.nn.Conv2d(in_channels=6, out_channels=16, kernel_size=[5, 5], stride=[7, 5], padding=[0, 0], dilation=[1, 1], groups=1, bias=True)
        self.relu2_mutated = torch.nn.ELU(alpha=0.1)
        self.pool2 = torch.nn.MaxPool2d(kernel_size=[2, 2], stride=[2, 2], padding=[0, 0], dilation=1, ceil_mode=False)
        self.flatten = torch.nn.Flatten()
        self.linear1_mutated = torch.nn.Linear(in_features=16, out_features=120)
...
def go():
    try:
        model = Model_WsMsDPQSJciomCH995XFEZngH7RTSMqu().to('cuda')
