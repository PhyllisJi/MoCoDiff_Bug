...
class Model_1715506754(nn.Layer):
    ...
    def forward(self, input):
        ...
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/conv2_output.npz",conv2_output.cpu().detach().numpy())
        relu2_output = self.relu2_mutated(conv2_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/relu2_output.npz",relu2_output.cpu().detach().numpy())
        ...
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/fc1_output.npz",fc1_output.cpu().detach().numpy())
        relu3_output = self.relu3_mutated(fc1_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/relu3_output.npz",relu3_output.cpu().detach().numpy())
        fc2_output = self.linear2_mutated(relu3_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/fc2_output.npz",fc2_output.cpu().detach().numpy())
