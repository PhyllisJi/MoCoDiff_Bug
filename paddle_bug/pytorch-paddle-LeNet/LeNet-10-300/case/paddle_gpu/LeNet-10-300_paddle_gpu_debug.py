...
output_dir = "./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/"
...
class Model_1715506754(nn.Layer):
    def __init__(self):
        ...
        self.conv1_mutated = paddle.nn.Conv2DTranspose(in_channels=1, out_channels=6, kernel_size=[5, 5], stride=[1, 1],
        ...
        self.conv2_mutated = paddle.nn.Conv2D(in_channels=6, out_channels=16, kernel_size=[5, 5], stride=[1, 1],
                                              padding=[1, 1], dilation=[1, 1], groups=1, bias_attr=None)
    ...
    def forward(self, input):
        ...
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/conv2_output.npz",
                 conv2_output.cpu().detach().numpy())
        relu2_output = self.relu2_mutated(conv2_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/relu2_output.npz",
                 relu2_output.cpu().detach().numpy())
        maxpool2_output = self.pool2(relu2_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/maxpool2_output.npz",
                 maxpool2_output.cpu().detach().numpy())
        flatten_output = self.flatten(maxpool2_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/flatten_output.npz",
                 flatten_output.cpu().detach().numpy())
        fc1_output = self.linear1_mutated(flatten_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/fc1_output.npz",
                 fc1_output.cpu().detach().numpy())
        relu3_output = self.relu3_mutated(fc1_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/relu3_output.npz",
                 relu3_output.cpu().detach().numpy())
        fc2_output = self.linear2_mutated(relu3_output)
        np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/fc2_output.npz",
...
print(output)
np.savez("./pytorch-paddle-LeNet/LeNet-10-300/case/paddle_gpu/layer_outputs/output.npz", output)
